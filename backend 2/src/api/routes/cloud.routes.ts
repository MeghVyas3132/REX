...
// GET /api/cloud - Get all cloud infrastructure nodes
router.get('/', async (req, res) => {
  try {
    ...
    logger.error('Failed to fetch cloud nodes', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to fetch cloud nodes'
    });
  }
});

// GET /api/cloud/categories - Get cloud nodes grouped by category
...
// GET /api/cloud/:id - Get specific cloud node definition
...
// POST /api/cloud/validate - Validate cloud node configuration
